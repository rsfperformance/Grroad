<div class="calc" style="display: {{ $display }}">
    ...
    <form class="calc__container">
                            ...
                            <div class="two__info">
                                ...
                                <div class="two__info-checkboxes">
                                    <label for="check-1" @if($price == 1) class="checkedbox" @endif>
                                        ...
                                        {!! __('asd.<p>Со снятием старого покрытия</p>') !!}
                                    </label>
                                    <label for="check-2" @if($price == 2) class="checkedbox" @endif>
                                        ...
                                        {!! __('asd.<p>Без снятия старого покрытия</p>') !!}
                                    </label>
                                </div>
                                <div class="two__info-bottom">
                                    <div class="left">
                                        <p>Итого:{{ __('asd.Отправить') }}</p>
                                        <span>
                                            @if($price == 1) {{ $data->full_price }} @endif
                                            @if($price == 2) {{ $data->price }} @endif
                                            @if($price == null) 0 @endif</span>
                                    </div>
                                    <button wire:click="application()" type="button">{{ __('asd.Оставить заявку') }}</button>
                                </div>
                            </div>
                    @endforeach
                        </div>
            ...
            <div class="stages__container {{ $display_third}}">
                <div class="stages__container-three">
                    <div class="three__form">
                        <h3>{{ __('asd.ОСТАВЬТЕ ЗАЯВКУ') }}</h3>
                        <input value="{{ $service }}" name="service" required hidden>
                        <input value="{{ $price }}" name="price_type" required hidden>
                        <input type="text" placeholder="{{ __('asd.имя фамили') }}я" name="name" wire:model="name">
                        <input type="text" placeholder="{{ __('asd.номер телефона') }}" name="phone" wire:model="phone">
                        <textarea name="" id="" cols="30" placeholder="{{ __('asd.сообщение') }}" rows="10" name="message" wire:model="message"></textarea>
                        <a wire:click="save()"><button type="button" wire:click="save()">{{ __('asd.Оставить заявку') }}</button></a>
                    </div>
                    <div class="three__contact">
                        <h3>{{ __('asd.свяжитесь с нами') }}</h3>
                        <div>
                            <p>{{ __('asd.Адрес:') }}</p>
        ...
        <div class="calc__container-success" style="display: {{ $display2 }}">
            ...
            <button type="button" class="calc__container-stage" wire:click="success()">{{ __('asd.На главную') }}</button>
        </div>
    </form>
</div>
